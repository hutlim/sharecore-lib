...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new sharecore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new sharecore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = sharecore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new sharecore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function sharecoreTest() {
      block1 = sharecore.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('sharecore', sharecoreTest, {maxTime: maxTime});
