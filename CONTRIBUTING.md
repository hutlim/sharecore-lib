Contributing to Sharecore
=======

We're working hard to make *sharecore* the most powerful JavaScript library for working with sharecoin. Our goal is to have *sharecore* be a library that can be used by anyone interested in sharecoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in sharecore that any change must adhere.
...
When possible, sharecore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Sharecore methods. For example, calling `JSON.stringify` on a Sharecore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'sharecore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the sharecore object.

DO:
```javascript
var sharecore = require('../');
var PublicKey = sharecore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add sharecoin-project git@github.com:sharecoin-project/sharecore.git
git pull --rebase sharecoin-project master
```
...
```
Finally go to [github.com/hutlim/sharecore](https://github.com/hutlim/sharecore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of sharecore.

If you have any questions feel free to post them to
[github.com/hutlim/sharecore/issues](https://github.com/hutlim/sharecore/issues).
